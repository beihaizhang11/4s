...
**邮件收件人顺序**:
```
TO: [首位固定组(可多个)] → [sender_mail] → [动态查询的staff_email] → [末尾固定组(可多个)]
CC: [全部固定]
```
...
4. **动态收件人**: 
   - sender_mail: 任务发起人邮箱（来自tasks表）
   - staff_email: 合并所有Task ID关联的员工邮箱（来自tasks_staff表）
   - 自动去重
